...
Full-text search is native PostgreSQL — there is no Elasticsearch and no reindexing.
Searchable tables have a generated **STORED** `search_vector tsvector` column plus a GIN
index, added via migration (see `20260810100500000_add_search_vector_columns.sql` for the
pattern: `'simple'` config, `id::text` included, secrets/encrypted content excluded).
Because the column is generated, it updates automatically with every row write — there is
nothing to keep in sync in Crystal. The query side lives in rest-api's index routes
(parameterized `search_vector @@ to_tsquery('simple', ?)` conditions), not in this repo.
To make a new model searchable, add its `search_vector` column in a migration; no model
code changes are needed (pg-orm ignores unmapped columns).
...
If a new model should be searchable, remind the user that the matching rest-api index
route needs a search-capable query (there is no separate indexing service to update).
